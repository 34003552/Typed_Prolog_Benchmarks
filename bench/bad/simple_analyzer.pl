...
:- type clause ---> (any :- any) ; (any :- node(integer, any, node, node)).
:- type stree ---> stree(any, clause, any, any, any).
:- type node(T0, T1, T2, T3) ---> node(T0, T1, T2, T3) ; 'leaf'.
:- type node ---> 'leaf' ; to_node(node(integer, any, node, node)).
%:- type predicate ---> (list(char)/integer).
:- type functor_t ---> node(integer, any, node, node) ; (functor_t = functor_t) ; to_functor_t(any).
...
:- pred benchmark(empty, node(integer,any,node,node)).
...
:- pred analyze_strees(list(any), node(integer,any,node,node)).
...
:- pred analyze_closure(list(any), node(integer,any,node,node), node(integer,any,node,node)).
...
:- pred analyze_closure(list(any), node(integer,any,node,node), node(integer,any,node,node), integer).
...
:- pred init_strees(list(any), node(integer,any,node,node), node(integer,any,node,node)).
init_strees([],_4,_4) :-
   true.
...
:- pred init_conj(alpha_t, node(integer,any,node,node), node(integer,any,node,node)).
...
:- pred init_disj(alpha_t, node(integer,any,node,node), node(integer,any,node,node)).
...
:- pred init_goal(functor_t, node(integer,any,node,node), node(integer,any,node,node)).
init_goal(_3,_4,_5) :-
   call_p(_3),
   !,
   functor(_3,_12,_13),
...
:- pred traverse_strees(list(any), node(integer,any,node,node), node(integer,any,node,node), integer, integer).
...
:- pred traverse_disj(functor_t, alpha_t, node(integer,any,node,node), node(integer,any,node,node), integer, integer).
...
:- pred traverse_conj(functor_t, node(integer,any,node,node), node(integer,any,node,node), node(integer,any,node,node), integer, integer).
...
:- pred traverse_conj(alpha_t, node(integer,any,node,node), node(integer,any,node,node), integer, integer, list(any), list(any), list(any), list(any), list(any), list(any)).
...
:- pred update_goal(functor_t, list(any), node(integer,any,node,node), node(integer,any,node,node), integer, integer, list(any), list(any), list(any), list(any), list(any), list(any)).
...
:- pred update_table(any, functor_t, node(integer,any,node,node), node(integer,any,node,node), integer, integer).
...
:- pred create_new_call(integer, integer, list(any), list(any), list(any), list(any), list(any), functor_t, functor_t).
...
:- pred lattice_modes_call(gamma_t, node(integer,any,node,node), clause).
lattice_modes_call(Na/Ar, Table, (Head:-Formula)) :-
	functor(Head, Na, Ar),
	get(Table, Na/Ar, Value),
	lattice_modes_call(1, Ar, Value, Head, Formula, true).

:- pred lattice_modes_call(integer, integer, functor_t, functor_t, any, any).
...
:- pred lattice_modes_arg(state, any, any, any).
...
:- pred get(node(integer,any,node,node), gamma_t, functor_t).
get(node(N,to_functor_t(W),to_node(L),to_node(R)), to_gamma_t(I), V) :- get(N, W, L, R, to_gamma_t(I), V).

:- pred get(integer, functor_t, node(integer,any,node,node), node(integer,any,node,node), gamma_t, functor_t).
get(N, V, _, _, to_gamma_t(I), V) :- I=N, !.
get(N, _, L, R, to_gamma_t(I), V) :-
	compare(Order, I, N),
	get(Order, to_gamma_t(I), V, L, R).

:- pred get(cmp, gamma_t, functor_t, node(integer,any,node,node), node(integer,any,node,node)).
get(<, to_gamma_t(I), V, L, _) :- get(L, to_gamma_t(I), V).
get(>, to_gamma_t(I), V, _, R) :- get(R, to_gamma_t(I), V).

:- pred set(node(integer,any,node,node), integer, functor_t, node(integer,any,node,node)).
...
:- pred set_2(cmp, integer, functor_t, functor_t, node(integer,any,node,node), node(integer,any,node,node), functor_t, node(integer,any,node,node), node(integer,any,node,node)).
...
:- pred seal(node(integer,any,node,node)).
seal(leaf).
...
:- pred membership_flag(functor_t, list(any), yn_t).
membership_flag(X, Set, yes) :- inv(to_functor_t(X), Set), !.
membership_flag(_, _et,  no).

:- pred ground_flag(functor_t, list(any), yn_t).
ground_flag(X, Ground, yes) :- varset(X, Set), subsetv(Set, Ground), !.
ground_flag(_, _round,  no).

:- pred get_entry_modes(state, functor_t, any, list(any)).
get_entry_modes(Type, Head, Value, TypeSet) :-
	functor(Head, _, Ar),
	get_entry_modes(Type, 1, Ar, Head, Value, Bag),
	sort(Bag, TypeSet).

:- pred get_entry_modes(state, integer, integer, functor_t, any, list(any)).
...
:- pred var_args(functor_t, list(any)).
var_args(Goal, Set) :-
	functor(Goal, _, Ar),
...
:- pred goal_dupset(functor_t, list(any)).
goal_dupset(Goal, DupSet) :-
	goal_dupset_varbag(Goal, DupSet, _).

:- pred goal_dupset_varset(functor_t, list(any), list(any)).
goal_dupset_varset(Goal, DupSet, VarSet) :-
	goal_dupset_varbag(Goal, DupSet, VarBag),
	sort(VarBag, VarSet).

:- pred goal_dupset_varbag(functor_t, list(any), list(any)).
goal_dupset_varbag(Goal, DupSet, VarBag) :-
	varbag(Goal, VarBag),
...
:- pred set_command(any, list(any), list(any)).
...
:- pred table_command(accessor, node(integer,any,node,node), node(integer,any,node,node)).
...
:- pred inv(any, list(any)).
inv(A, [B|S]) :-
	compare(Order, A, B),
	inv_2(Order, A, S).

:- pred inv_2(cmp, any, list(any)).
inv_2(=, _, _).
inv_2(>, A, S) :- inv(A, S).

:- pred intersectv(list(any), list(any), list(any)).
intersectv([], _, []).
intersectv([A|S1], S2, S) :- intersectv_2(S2, A, S1, S).

:- pred intersectv_2(list(any), any, list(any), list(any)).
intersectv_2([], _, _, []).
intersectv_2([B|S2], A, S1, S) :-
	compare(Order, A, B),
	intersectv_3(Order, A, S1, B, S2, S).

:- pred intersectv_3(cmp, any, list(any), any, list(any), list(any)).
