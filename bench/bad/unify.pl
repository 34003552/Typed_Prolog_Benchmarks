...
:- type h_t ---> 'h'.
:- type unk_t0 ---> any.
:- type unk_t1 ---> any ; (termtag_t ^ unk_t1) ; (h_t + integer) ; to_unk_t1(list(any)).
:- type termtag_t ---> tstr ; tlst ; tatm ; tvar.

:- trust_pred atomic(unk_t1).
:- trust_pred \+(any).

:- pred data(empty).
data(_).

:- pred benchmark(empty, integer).
benchmark(_Data, Size) :-
	u(X, [1,_], [X], Code),
	size(Code, 0, Size).

% Unify variable X with term T and write the result:
:- pred u(alpha_t, list(integer), list(alpha_t), any).
...
:- pred unify(alpha_t, list(integer), list(alpha_t), any, any, list(any)).
...
%:- pred init(any, any, any, any, any, any).
...
%:- pred unify_var(any, any, any, any).
...
%:- pred unify_readmode(any, any, any, any, any, any, list(any)).
...
%:- pred unify_args(any, any, any, any, any, any, any, any).
...
%:- pred unify_arg(any, any, any, any, any, any, any, any).
...
%:- pred unify_writemode(any, any, any, any, any, any, list(any)).
...
%:- pred unify_block(any, any, any, any, any, list(any)).
...
%:- pred block(any, any, any, any, any, list(any)).
...
%:- pred block_args(integer, integer, any, any, any, any, any, any, any).
...
%:- pred make_slots(integer, integer, any, any, any, any, any).
...
:- pred in(unk_t0, list(unk_t0)).
in(A, [B|S]) :-
        compare(Order, A, B),
        in_2(Order, A, S).

:- pred in_2(cmp, unk_t0, list(unk_t0)).
in_2(=, _, _).
in_2(>, A, S) :- in(A, S).

:- pred incl(unk_t0, list(unk_t0), list(unk_t0)).
incl(A, S1, S) :- incl_2(S1, A, S).

:- pred incl_2(list(unk_t0), unk_t0, list(unk_t0)).
incl_2([], A, [A]).
incl_2([B|S1], A, S) :-
        compare(Order, A, B),
        incl_3(Order, A, B, S1, S).

:- pred incl_3(cmp, unk_t0, unk_t0, list(unk_t0), list(unk_t0)).
incl_3(<, A, B, S1, [A,B|S1]).
incl_3(=, _, B, S1, [B|S1]).
incl_3(>, A, B, S1, [B|S]) :- incl_2(S1, A, S).

:- pred my_compound(unk_t1).
my_compound(X)  :- nonvar(X), \+atomic(X).
:- pred cons(unk_t1).
cons(X)      :- nonvar(X), X=to_unk_t1([_|_]).
:- pred structure(unk_t1).
structure(X) :- my_compound(X), \+X=[_|_].

:- pred termtag(unk_t1, termtag_t).
