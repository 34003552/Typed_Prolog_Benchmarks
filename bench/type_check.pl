...
tc_version('$Id: type_check.pl,v 1.52 2010-06-16 03:51:31 toms Exp $').
...
type_check_term_(Term,Type,EnvIn,EnvOut) :-
	nonvar(Type),
	functor(Type,pred,Arity),
	!,
	Type =.. [_|ArgTypes], 
	Term =.. [Functor|Args],
	length(DummyArgs,Arity),
	append(Args,DummyArgs,FullArgs),
	Goal =.. [Functor|FullArgs],
	pairup(DummyArgs,ArgTypes,ExtraEnv),
	append(ExtraEnv,EnvIn,Env),
	type_check_control(Goal,top,Env,EnvOut).
...
type_check_clauses([Clause|Clauses],Stats0,Stats) :-
		...
		( %trace,
	     ...
	     ,  error(type_error(_Term,_ExpType,_InfType))
...
filter([],_Pred,[]).
filter([X|Xs],Pred,Result) :-
	( call(Pred,X) ->
		Result = [X|More],
		filter(Xs,Pred,More)
	;
		filter(Xs,Pred,Result)
	).

check_ambiguous_types(Head) :-
	term_variables(Head,TyVars),
	filter(TyVars,is_ambiguous_numeric_type,AmbTyVars),			
	( AmbTyVars == [] ->
		true
	;
		copy_term_nat(Head-AmbTyVars,Head1-AmbTyVars1),
		numbervars(Head1-AmbTyVars1,0,_),
		write( '%------------------------------------------------------------------------------%'), nl,
		format('TYPE ERROR: Ambiguous numeric types `~p\'.\n',[AmbTyVars1]), 
		format('           in predicate `~p\'\n',[Head1]),
		format('            types should be one of `integer\' or `float\' \n',[]),
		throw(type_error)
	).
...
numeric_type(Type) :-
	( nonvar(Type) ->
		check_numeric_type(Type)
	;
		put_attr(Type,numeric_type,x)
	).

numeric_type:attr_unify_hook(_,Other) :-
	( nonvar(Other) ->
		check_numeric_type(Other)
	;
		put_attr(Other,numeric_type,_)
	).
...
is_ambiguous_numeric_type(Type) :- var(Type), get_attr(Type,numeric_type,_).
...
type_check_expression(log(Exp),Type,Env1,Env2) :- !,
	equate_types(Type,float),
	type_check_expression(Exp,Type,Env1,Env2).	
type_check_expression(float(Exp),Type,Env1,Env2) :- !,
...
type_check_expression(integer(Exp),Type,Env1,Env2) :- !,
	equate_types(Type,integer), % explicit conversion from float to integer
	type_check_expression(Exp,float,Env1,Env2).	
...
head_error(Term,ExpType,InfType,Head,Body) :-
	...
	portray_clause((Head :- Body)),
        term_type_error(Term,ExpType,InfType).

less_polymorphic_error(InfSig,DecSig) :-
	copy_term_nat(InfSig,InfSig1),
	copy_term_nat(DecSig,DecSig1),
	numbervars(InfSig1,0,_),
	numbervars(DecSig1,0,_),
	write( '%------------------------------------------------------------------------------%'), nl,
...
pairup([],[],[]).
pairup([X|Xs],[Y|Ys],[X-Y|Rest]) :- pairup(Xs,Ys,Rest).
% }}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
% Goal expansion {{{
...
% Strip modes {{{
strip_modes(FullSignature,Signature) :-
	FullSignature =.. [Functor|FullArgs],
	maplist(strip_mode,FullArgs,Args),
	Signature =.. [Functor|Args].

strip_mode(FullArg,Arg) :-
	( var(FullArg) 		->	Arg = FullArg
	; FullArg = '+'(Arg)	-> 	true
	; FullArg = '-'(Arg)	->	true
	; FullArg = '?'(Arg) 	-> 	true
	; 				Arg = FullArg
...
user:term_expansion((:- pred FullSignature),
		    [ Clause
		    ]) :-
	strip_modes(FullSignature,Signature),
	signature_clause(Signature,Clause).
user:term_expansion((:- trust_pred FullSignature),
		    [ SignatureClause
