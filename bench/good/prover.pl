...
:- type empty.
:- type alpha_t ---> refuted ; fs(list(beta_t), list(beta_t), list(beta_t), list(beta_t)).
:- type beta_t ---> a ; b ; c ; to_be ; +beta_t ; -beta_t ; beta_t & beta_t ; beta_t # beta_t.
...
% problem set
:- pred problem(integer, beta_t, beta_t).
problem( 1, -a, +a).
problem( 2, +a, -a & -a).
...
:- pred add_conjunction(beta_t, beta_t, alpha_t).
add_conjunction(F, G, Set) :-
	expand(F, Set, Mid),
	expand(G, Mid, New),
	refute(New).

:- pred expand(beta_t, alpha_t, alpha_t).
...
:- pred includes(list(beta_t), beta_t).
includes([Head|_], Head) :- !.
includes([_|Tail], This) :- includes(Tail, This).

:- pred extend(beta_t, list(beta_t), list(beta_t), list(beta_t), alpha_t, alpha_t).
