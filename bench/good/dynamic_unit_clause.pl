...
:- trust_pred assertz(pred).
:- trust_pred retractall(pred).
...
:- pred dynamic_clause(integer, foo, integer).
:- dynamic dynamic_clause/3.

:- pred init_dynamic_clause1(list(integer)).
...
init_dynamic_clause1([Key|Keys]) :-
   ...
   assertz(dynamic_clause(Key, 'Foo', Value)),
   init_dynamic_clause1(Keys).

:- pred init_dynamic_clause(list(integer)).
init_dynamic_clause(Data) :-
   ...
   retractall(dynamic_clause(_,_,_)),
...
:- pred dynamic_unit_clause(list(integer), list(integer)).
dynamic_unit_clause([], []).
dynamic_unit_clause([X|Xs], [Y|Ys]) :-
   dynamic_clause(X, 'Foo', Y),
